...
from panda_gym.envs.robots.panda import Panda, MobilePanda
...
class PandaMobilePickAndPlaceEnv(RobotTaskEnv):
    """Pick and Place task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = MobilePanda(sim, block_gripper=False, base_position=np.array([-1.5, -0.2, -0.4]), control_type=control_type)
...
class PandaMobileReachEnv(RobotTaskEnv):
    """Reach task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = MobilePanda(sim, block_gripper=False, base_position=np.array([-1.5, 0.0, -0.4]), control_type=control_type)
        task = Reach(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, mobile=True)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaMobileSlideEnv(RobotTaskEnv):
    """Slide task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = MobilePanda(sim, block_gripper=True, base_position=np.array([-1.5, 0.0, -0.4]), control_type=control_type)
        task = Slide(sim, reward_type=reward_type,goal_xy_range=0.3,goal_x_offset=0.0,goal_y_offset=0.4, obj_y_offset = 0.4,obj_xy_range=0.15,mobile=True)
        super().__init__(
            robot,
            task,
            render_width=render_width,
